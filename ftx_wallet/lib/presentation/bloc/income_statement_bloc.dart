...
class IncomeStatementBloc
  ...
  IncomeStatementBloc() : super(IncomeStatementInitial()) {
    ...
    this._getIncomeStatementUsecase =
        GetIncomeStatementUsecase(_walletRepository);
  }
  ...
  Stream<IncomeStatementState> mapEventToState(
    ...
    if (event is GetIncomeStatementEvent) {
      ...
      final result = await _getIncomeStatementUsecase(NoParams());
      yield result.fold((failure) => IncomeStatementError(failure.message),
          (incomeStatement) => IncomeStatementLoaded(incomeStatement));
    }
  }
}
